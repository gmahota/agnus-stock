      ...
      <>
        {state.map((item, i) => (
          <div
            ...
            <div className="ml-auto mr-1 lg:mr-2 whitespace-nowrap text-xs text-gray-500">{item.date}</div>
            <button
              className="btn btn-circle"
              onClick={() => dispatch({type: 'remove', id: item.id})}>
              <FiX size={16} className="stroke-current text-gray-500" />
            </button>
          </div>
        ))}
      </>
      <form onSubmit={handleSubmit}>
        <div className="form-element">
          <input
            ...
            name="todo"
            type="text"
            className="form-input text-sm bg-white dark:bg-gray-800 dark:border-gray-800"
            placeholder="Add new..."
          />
        </div>
      </form>
